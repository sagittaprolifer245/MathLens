...
1. 读取分镜脚本的音频清单部分
2. 验证音频文件存在且时长正常
3. 读取 audio_info.json 中的 sync_points（句级同步点）
4. 更新分镜脚本的时长列
5. 打印各幕同步点摘要，方便在 play_scene_X() 中使用 wait_for_narration()
6. 如果缺少长度或长度异常，报错提醒
...
输出：
    - 更新后的分镜.md（填充时长列）
    - audio/audio_info.json（由 generate_tts.py 生成，本脚本只验证）
"""
...
def print_sync_points_summary(audio_dir, audio_list):
    """
    从 audio_info.json 读取 sync_points 并打印摘要。
    方便开发者在 play_scene_X() 中选择 wait_for_narration() 的关键词。
    """
    ...
    has_sync = any(f.get('sync_points') for f in files)
    if not has_sync:
        print("\n💡 提示：audio_info.json 中无 sync_points。")
        print("   重新运行 generate_tts.py 可自动生成句级同步点。")
        return

    print("\n" + "=" * 50)
    print("🎯 同步点摘要（用于 wait_for_narration）")
    print("=" * 50)

    for item in files:
        scene_num = item.get('scene', '?')
        duration = item.get('duration', 0)
        sp_list = item.get('sync_points', [])
        if not sp_list:
            continue

        print(f"\n  第{scene_num}幕 ({duration:.1f}s):")
        for sp in sp_list:
            t = sp.get('time', 0)
            text = sp.get('text', '')[:35]
...
def main():
    ...
    # 打印同步点摘要（从 audio_info.json 读取）
    print_sync_points_summary(audio_dir, updated_list)
